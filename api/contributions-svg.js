...
function generateSVG(weeks, theme, lang) {
  ...
  monthMarkers.forEach(({ m, wi }) => {
    const x = paddingLeft + wi * step;
    monthLabels += `<text x="${x}" y="${paddingTop - 8}" font-size="9" fill="${labelColor}" font-family="${FONT_FAMILY}">${MONTHS[m]}</text>`;
  });

  let dayLabels = '';
  DAYS.forEach((d, i) => {
    if (d) dayLabels += `<text x="${paddingLeft - 4}" y="${paddingTop + i * step + cellSize - 2}" font-size="8" fill="${labelColor}" font-family="${FONT_FAMILY}" text-anchor="end">${d}</text>`;
  });
